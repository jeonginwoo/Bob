...
import { 
    Box, TextField, Button, TableContainer, Table, TableHead, TableBody, TableRow, TableCell, Paper, 
    useTheme 
} from '@mui/material';
        ...
        if (currentSpeed.current <= 0) {
            ...
            for (let i = 0; i < product.length; i++) {
                ...
                if (winningAngleRad >= start && winningAngleRad < start + arc) {
                    setTimeout(() => alert(`오늘의 점심은?! 🎉 ${product[i].name} 🎉`), 100);
                    break;
                }
                start += arc;
            }
    ...
    return (
        <Box sx={{ p: 2 }}>
            <Box sx={{ position: 'relative', width: '100%', maxWidth: 300, mx: 'auto' }}>
                <canvas ref={canvasRef} style={{ display: 'block', margin: 'auto', width: '100%', height: 'auto' }}></canvas>
                <Box sx={{
            ...
            <TableContainer component={Paper} sx={{ mt: 3, backgroundColor: theme.palette.background.paper }}>
                <Table size="small">
                    <TableHead>
                        <TableRow>
                            <TableCell sx={{ borderColor: theme.palette.text.secondary, color: theme.palette.secondary.main, textAlign: 'center', width: '25%' }}>메뉴</TableCell>
                            <TableCell sx={{ borderColor: theme.palette.text.secondary, color: theme.palette.secondary.main, textAlign: 'center' }}>비중</TableCell>
                            <TableCell sx={{ borderColor: theme.palette.text.secondary, color: theme.palette.secondary.main, textAlign: 'center' }}>삭제</TableCell>
                        </TableRow>
                    </TableHead>
                    <TableBody>
                        {product.map((item, index) => (
                            <TableRow key={index}>
                                <TableCell sx={{ borderColor: theme.palette.text.secondary, color: theme.palette.text.primary, textAlign: 'center', width: '25%' }}>{item.name}</TableCell>
